...
import { LogOut, Package, ShoppingCart, FileText, Plus, CreditCard as Edit2, Trash2, Save, X } from 'lucide-react';
...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  const loadData = async () => {
    if (activeTab === 'products') {
      ...
      const { data } = await supabase.from('site_content').select('*').order('page');
  ...
  const handleSaveContent = async (id: string) => {
    await supabase.from('site_content').update({ content: formData.content }).eq('id', id);
    setEditingId(null);
    setFormData({});
    loadData();
  };
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {products.map((product) => (
                <div key={product.id} className="bg-white rounded-xl shadow p-6 flex items-center gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-4 mt-2">
                      <span className="text-orange-600 font-bold">{product.price.toLocaleString()} CFA</span>
                      <span className="text-gray-500">Stock: {product.stock}</span>
                      {product.is_featured && (
                        <span className="text-green-600 font-semibold">⭐ En vedette</span>
                      )}
                    </div>
                  </div>
            ...
            {orders.map((order) => (
              <div key={order.id} className="bg-white rounded-xl shadow p-6">
                ...
                <div className="border-t pt-4">
                  <p className="text-gray-700"><strong>Produit:</strong> {order.product_name}</p>
                  <p className="text-gray-700"><strong>Quantité:</strong> {order.quantity}</p>
          ...
          <div className="space-y-4">
            {siteContent.map((content) => (
              <div key={content.id} className="bg-white rounded-xl shadow p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg font-bold text-gray-800 capitalize">{content.page}</h3>
